...
namespace TeamJRPG
{
    public class ItemHolder : UIComposite
    {
        ...
        public ItemHolder(Item item, Vector2 startPosition)
        {
            ...
            Frame frame = new Frame(position, new Vector2(item.sprite.srcRect.Width * scale + padding.X, item.sprite.srcRect.Height * scale + padding.Y));
            frameSize = frame.frameSize;

            children.Add(frame);

            ImageHolder icon = new ImageHolder(item.sprite, new Vector2(position.X + padding.X + 16 / 4, position.Y + padding.Y), Color.White, new Vector2(scale * 1.5f, scale * 1.5f), null);
            ...
            if (itemBox.Contains(cursorPointF))
            {
                ...
                if (Globals.inputManager.IsMouseButtonClick(InputManager.MouseButton.Right))
                {
                    if (!buttonMenuOn)
                    {
                        ...
                        {
                            buttons[i] = new TextButton(textButtonTexts[i], new Vector2(adjustedPos.X, adjustedPos.Y + ((Globals.assetSetter.fonts[0].MeasureString("K").Y + 28) * i)), 0, buttonIdList[i], Color.White);
                        }
                ...
                {
                    if (!Globals.inventoryHandler.IsDraggingItemInInventory)
                    {
                    ...
                    if (!dragOn && !Globals.inventoryHandler.IsDraggingItemInInventory)
                    {
                        Vector2 adjustedPos = new Vector2(Globals.camera.viewport.Width / 2, Globals.camera.viewport.Height / 2);
                        drag = new ImageHolder(item.sprite, adjustedPos, Color.White, new Vector2(2, 2), null);
                        ...
                        IsDragging = true;
                        dragOn = true;
                        Globals.inventoryHandler.IsDraggingItemInInventory = true;
                    }
                }
                else
                {
                    ...
                    Globals.inventoryHandler.IsDraggingItemInInventory = false;
