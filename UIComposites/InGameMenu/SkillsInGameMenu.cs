...
namespace TeamJRPG
{
    public class SkilsInGameMenu : UIComposite
    {
        public SkilsInGameMenu()
        {
            ...
            Button leftArrow = new Button(buttonTexture, new Vector2(Globals.camera.viewport.Width / 2 + currentCharacter.position.X - buttonTexture.Width - buttonOffset.X, framePos.Y + currentCharacter.texture.Height / 2 - buttonTexture.Height / 3), 13);
            for (int i = 0; i < leftArrow.components.Count; i++)
            {
                leftArrow.components[i].spriteEffects = SpriteEffects.FlipHorizontally;
            }

            Button rightArrow = new Button(buttonTexture, new Vector2(Globals.camera.viewport.Width / 2 + currentCharacter.position.X + currentCharacter.texture.Width + buttonOffset.X, framePos.Y + currentCharacter.texture.Height / 2 - buttonTexture.Height / 3), 14);
